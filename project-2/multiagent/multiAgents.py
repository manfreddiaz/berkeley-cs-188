...
def betterEvaluationFunction(currentGameState):
    ...
    # Calculations
    ghost_distance = min(
        manhattan_distance(next_pacman_position, ghost.configuration.pos) for ghost in next_ghosts_states)
    closest_food_distance = min(
        manhattan_distance(next_pacman_position, nextFood) for nextFood in next_food) if next_food else 0
    scared_time = min(next_ghosts_scared_timers)
